"""
回归预测任务 - Random Forest 模型 (最终优化版)
针对弱相关数据集进行优化，旨在解决 R2 为负的问题。
"""
...
def process_features(df_features):
    ...
    numeric_cols = [str(i) for i in range(30)] # 列名现在是字符串
...
def train_and_evaluate():
    print("=" * 60)
    print("           回归预测任务 - Random Forest (最终优化版)")
    print("=" * 60)
    ...
    # 【关键】将所有列名转换为字符串，防止 TypeError
    all_data.columns = all_data.columns.astype(str)

    # 2. 特征工程 (添加统计特征)
    all_data = process_features(all_data)

    # 3. One-Hot 编码 (针对原第30列，现在列名是'30')
    cat_col = '30'
    print("\n[INFO] 正在应用 One-Hot 编码...")
    all_data = pd.get_dummies(all_data, columns=[cat_col], drop_first=False)
    ...
    # ==========================================
    # 第四部分：模型训练 (更换为 Random Forest)
    # ==========================================
    # Random Forest 对噪声更鲁棒
    ...
    param_grid = {
        ...
        'min_samples_split': [5, 10],   # 增加分裂所需的最小样本数，增强鲁棒性
        'max_features': ['sqrt', 'log2'] # 限制特征数量，进一步防止过拟合
    }
